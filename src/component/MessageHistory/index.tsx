                ...
                {props.history.map((item) => (
                    <section key={item.id} ref={parentRef}>
                        {item.status === "error" ? (
                            <Bubble
                                placement="start"
                                content={<Error question={item.question as string} retry={props.retry} />}
                                avatar={<RobotIcon />}
                            />
                        ) : item.type === "robot" && item.status ? (
                            <RobotMessage result={item.value} status={item.status} />
                        ) : (
                            <Bubble placement="end" content={item.value} avatar={<UserIcon />} />
                        )}
                    </section>
                ))}
            </Flex>
